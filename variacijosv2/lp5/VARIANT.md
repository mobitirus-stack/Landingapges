# lp5 — Pastelinis minimalizmas (v2, hero nuotrauka)

## Kryptis
`config/vizualines-kryptys-v2.md` → „## lp5 — Pastelinis minimalizmas" (antra grupė, lp7–lp10).
...
- Sėkmės būsena rodoma tik po sėkmingo pateikimo (forma pasislepia, `.lp5-success` atsidengia);
...
- **CTA spalva `#BE185D`, ne krypties bazinis `#DB2777`:** baltas tekstas ant `#DB2777` duoda
  ...
  `#BE185D` — 6.04:1. Tai tas pats atspalvių šeimos tonas (žr. `--lp5-accent` vs
  `--lp5-accent-strong` kintamuosius `style.css`), taikomas tik CTA fonui/ženkliukams; kortelės
...
1. `.lp5-success{display:flex}` nustelbdavo `[hidden]` atributą, todėl sėkmės pranešimas rizikavo
   būti matomas nuo puslapio įkėlimo. Ištaisyta: pridėta globali `[hidden]{display:none!important}`
   taisyklė `style.css` viršuje (šalia jau buvusios specifiškesnės `.lp5-success[hidden]`
...
2. 360px plotyje `<select>` elementai galėjo priversti kortelę peržengti ekrano kraštą, nes grid
   ...
   `min-width:0` ir ant `.lp5-select-wrap` (grid elemento), ir ant pačių `select` elementų.
