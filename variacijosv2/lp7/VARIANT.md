# lp7 — Auksinės valandos hero nuotrauka
...
**Pastaba dėl nuotraukos:** `config/vizualines-kryptys-v2.md` lp7–lp10 „ANTRA GRUPĖ" skyrius
...
saulėlydžio tonai) paimta iš `vizualines-kryptys-v2.md` lp7 aprašymo ir pritaikyta realiai
...
- `<label>`/`aria-label` kiekvienam formos laukui (select'ų vizualiai paslėpti `<label>`
  per `.lp7-sr-only`), `fieldset`/`legend` lyties grupei, `role="group"` datos eilutei.
...
Title/description/canonical → `https://vyrukambarys.lt/lp7`, OG + Twitter Card su
